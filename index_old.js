...
function cullVotes(game)
{
	...
	game.clients.forEach (c => {
			//console.log(c);
			if (clients[c.clientId] === clients[clientId].currentGameInfo.vote)
			{
				console.log("ITS A MATCH");
				//chat(game, null, "", "SELF VOTE BY " + clientId + ", -3 points lol");
				clients[clientId].currentGameInfo.selfVoted = true;
				//c.score -= 3;
			}	
			else if (clients[clientId].currentGameInfo.vote === null)
			{
				
				console.log("Didn't vote");
				clients[clientId].currentGameInfo.didNotVote = true;
				//chat(game, null, "", clientId + "didn't vote, -3 points lol");
				//c.score -= 3;
			}
			else
			{
				clients[clients[clientId].currentGameInfo.vote].votesReceived += 1;
			}	
	});
...
function calculateRoundResult(game)
{
	...
	game.clients.forEach (c => {
		let adjuster = clients[c.clientId].currentGameInfo.selfVoted || clients[c.clientId].currentGameInfo.didNotVote ? -6 : 0;
		clients[c.clientId].currentGameInfo.roundScore = (clients[c.clientId].currentGameInfo.votesReceived * 5) - adjuster;
	});
	...
	game.clients.forEach (c => {
		if (result.length == 0)
		{
			result.push({
				...
				"votesReceived" : clients[c.clientId].currentGameInfo.votesReceived,
				"didNotVote" : clients[c.clientId].currentGameInfo.didNotVote,
				"selfVoted" : clients[c.clientId].currentGameInfo.selfVoted,
				"roundScore" : clients[c.clientId].currentGameInfo.roundScore
			})
		}
		else
		{
			if (clients[c.clientId].currentGameInfo.votesReceived > result[0].votesReceived)
			{
				result.unshift({
					"acronym": clients[c.clientId].currentGameInfo.play,
					"votesReceived" : clients[c.clientId].currentGameInfo.votesReceived,
