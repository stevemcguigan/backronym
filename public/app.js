...
window.mobileCheck = function() {
  ...
  clog("mobilecheck returned " + check, 1); 
...
function loadUser()
{
	...
	if (user == null)
	{
		clog("no user found", 0)
		createNewUser();
	} else {
		clog("found an old user!", 0)
		nick = user.nick;
    if (!user.localId)
    {
      user.localId = guid();
      clog("Found a legacy user with no guid, adding. " + user.localId, 0);
      saveUser();
    } 
	}	
...
function createNewUser()
{
    ...
    user = {
      "localId": null, 
      "nick": `Player${randomInt(1000,9999)}`,
      "theme": 0,
      "wins" : 0,
      "theme" : 0,
      "created" : 0
    ...
    create_new_modal({
      ...
      prompt: "welcome!",
      textbox: "name",
      detail_text: "<span style='color: olivedrab'>Backronym is played with others. So to get started, what nickname do you want to use when you're playing the game?</span>",
